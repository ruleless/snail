...
static bool s_bNetworkInitted = false;

static ObjectPool<EndPoint> s_ObjPool("EndPoint");
...
int EndPoint::getBufferSize(int optname) const
{
	...
	int rberr = getsockopt(mSocket, SOL_SOCKET, optname, (char *)&recvbuf, &rbargsize);
...
{
	setsockopt(mSocket, SOL_SOCKET, optname, (const char*)&size, sizeof(size));
...
bool EndPoint::waitSend()
{
	...
	return select(mSocket+1, NULL, &fds, NULL, &tv) > 0;
}
