...
#include "WebSocketPacketFilter.h"
...
//-------------------------------------------------------------------------------------
EReason WebSocketPacketFilter::send(Channel * pChannel, PacketSender& sender, Packet * pPacket)
{
...
//-------------------------------------------------------------------------------------
EReason WebSocketPacketFilter::recv(Channel * pChannel, PacketReceiver & receiver, Packet * pPacket)
{
	while(pPacket->length() > 0)
	{
		if(fragmentDatasFlag_ == FRAGMENT_MESSAGE_HREAD)
		{
			if(pFragmentDatasRemain_ == 0)
			{
				...
				{
					fragmentDatasFlag_ = FragmentDataTypes_MessageDatas;
			...
			else
			{
				...
				if((int32)pPacket->length() >= pFragmentDatasRemain_)
				{
					...
					Assert(pFragmentDatasRemain_ == 0);
		...
		else
		{
			...
			EReason reason = PacketFilter::recv(pChannel, receiver, pTCPPacket_);
